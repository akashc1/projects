...
// Helper function for save and load that looks up a letter and returns its
...
// Load function, able to load in given a game type and an input file
// (decided to include game type because it was already decided at that point)
...
// Helper function that is used in makemove, returns 0 if there is no check
// 1 if they are moving into a check
// return 2 if they are still in check after the move
...
int ChessGame::make_move(Position start, Position end, int speak) {
  ...
  //If there is no piece there - check and make sure so that there isn't
  ...
  if(!to_move->valid_move_shape(start, end, trajectory)){
    ...
    // Checks if it is a pawn trying to move diagonally
    // sorts through the inputs as such
    ...
    // Check if the end piece is owned by the player
    // If yes then you are blocked, if not then capture it.
    bool occupied = false;
    if (get_piece(end) != nullptr) {
      occupied = true;
...
// Function that returns true if in checkmate
// Returns false if there is a single move that isnt in check
  ...
  for (unsigned int i = 0; i < _width; i++) {
	...
	for (unsigned int k = 0; k < _width; k++) {
	    ...
	    // Checks to see if it is a valid move
	      ...
	      int isBlocked = 0;
	      for (std::vector<Position>::iterator it = tempTraj.begin();
	                it != tempTraj.end(); it++) {
...
// Uses user input (that is 2 words long) and turns it into a move
// Collecting the information that the user gave and assigning
// Those values to positions
