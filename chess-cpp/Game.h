...
public:
    ...
    Game(unsigned int w = 8, unsigned int h = 8, int t = 0) :
    ...
    virtual void save(std::string filename, int gameType);
    ...
    //Parses the string for the moves - makes the moves if fitting
    int move_parsing(std::string entry);

    //Parses non-move strings
    int non_move_parsing(std::string entry);
    
    int isBlocked(std::vector<Position> trajectory);
    ...
    virtual int make_move(Position start, Position end, int speak) = 0;
